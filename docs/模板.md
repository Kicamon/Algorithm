...
```c++
struct Dijkstra {
        using queue_type = pair<long long, int>;
        const ll inf = 1e18;
        vector<ll> dis;
...
struct Johnson {
        ...
        Johnson(int size) {
                n = size;
                g.resize(size + 1), dis.resize(size + 1, inf), h.resize(size + 1, inf);
                for (int i = 1; i <= n; ++i) {
                        add_edge(0, i, 0);
                }
        }
